...
function generate_net(in_dim,out_dim)
{
  ...
  layer_defs.push({type:'fc', num_neurons:30, activation:'relu'});
  layer_defs.push({type:'softmax', num_neurons:out_dim});
...
function NNCore(in_dim,out_dim) {
  ...
  this.interact=(input)=>{
    ...
    //visualInput,health,energy,speedX,speedY,tentacles
    this.tmp_in_buff.w[0]=input.visualInput[0];
...
function NNCreature(body, CoreFunction)//body is the pysical(matter.js engine) body in env
{
  ...
  this.in_dim=this.visual.length+1+1+1+1+this.state.tentacleTar.length*this.state.tentacleTar[0].length;

  this.out_dim=tentacles.length*tentacles[0].length;//tentacles status
